...
ACCESS_TOKEN = "eyJhbGciOiJIUzUxMiJ9.eyJzdWIiOiIxOWVjYTg1Yy1iMWI3LTQyNTEtOThhNi0yYWJlYWRiNWZmNDEiLCJpYXQiOjE3NzA2MDM0NjUsImV4cCI6MTc3MDYwNzA2NX0.TlW3PKXpjxPuwGRjJXGUl-_vY5VAq73Wv5DTdjbGxq5skqmst1YxlnCW4ddW8vN4YebZZtuAurFUTvin5l_bbA"
VEHICLE_ID = "85e99715-6832-4241-9539-c370c084a0ee"
...
def main():
    ...
    for i in range(1, total_iterations + 1):
        # Random event count between 0 and 15
        event_count = random.randint(0, 15)
        # Offset to prevent overlap: spread out by 15 mins, starting from past
        offset = (total_iterations - i + 1) * 15 
        
        print(f"\n[{i}/{total_iterations}] Starting Iteration with {event_count} bad events (Offset {offset}m)...")
        
        trip_id = start_trip(VEHICLE_ID)
        if trip_id:
            # Generate start time (Naive Local)
            log_start_time = datetime.now() - timedelta(minutes=offset)
