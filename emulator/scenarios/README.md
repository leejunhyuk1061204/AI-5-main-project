...
## 20분 주행 시나리오 (run_emulator.py replay)

**진짜 운전처럼** 고정된 타임라인으로 20분 재생, 운전점수 감점 최소화용.
...
| 2 | 1~3분 | 시동 켬 + 대기 (공회전) |
| 3 | 3~18분 | 주행 (부드러운 가속→정속→감속, 급가속/급감속 없음) |
| 4 | 18~19분 | 시동 켬 + 대기 |
| 5 | 19~20분 | 시동 끔 |

### 사용 방법

1. CSV 생성 (최초 1회 또는 시나리오 수정 후):
   ```bash
   cd emulator/scenarios
   python generate_20min_drive.py
   ```
...
2. 에뮬레이터 실행 시 config에서 `vehicle_id: "DEV_004"` 사용 (기본값):
   ...
   ```
   또는: `python run_emulator.py --vehicle DEV_004`
