...
*하이퍼테이블(Hypertable)로 설정하며, 7일 보관 후 자동 삭제(Drop Chunk) 정책을 적용함.*
...
|:---|:---|:---|:---|:---|
| **Hot** | TimescaleDB | `obd_logs` | 7일 | 고해상도 초단위 데이터. 분석 후 삭제 |
...
- **실시간 로그 (`obd_logs`)**: 주행 중 1Hz로 쌓이는 데이터는 7일이 지나면 TimescaleDB의 `Drop Chunk` 기능을 통해 물리적으로 삭제하여 저장 공간을 최적화합니다.
