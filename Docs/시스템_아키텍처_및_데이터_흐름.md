...
- **In-Memory Store (Redis) 활용 범위**:
    - **Caching**: 자주 조회되는 표준 코드(DTC), 차량 제원 데이터 캐싱으로 DB 부하 감소.
    - **Session/Token Store**: Refresh Token 저장 및 관리, 로그아웃 처리(Blacklist).
    - **Real-time Buffer**: 실시간 모니터링을 위한 임시 데이터 버퍼링.
...
sequenceDiagram
    ...
    Note over Car, App: [Phase 1] 연결 및 핸드쉐이크
    App->>OBD: 블루투스 연결 요청 (RFCOMM)
    activate OBD
    OBD-->>App: 연결됨
    App->>OBD: AT 커맨드 전송 (초기화, 프로토콜 검색)
    OBD->>Car: 차량 프로토콜 식별 (CAN 등)
    Car-->>OBD: 프로토콜 확인
    OBD-->>App: 준비 완료

    Note over Car, TSDB: [Phase 2] 실시간 스트리밍 (1초 주기)
    loop 1초마다 반복
        App->>OBD: 데이터 요청 (RPM, 속도 등 PIDs)
        OBD->>Car: 실시간 데이터 획득
        Car-->>OBD: 16진수 원본 데이터 (예: 41 0C 1A F8)
        OBD-->>App: 원시 데이터 응답
        
        App->>App: 데이터 파싱 및 변환 (16진수 -> 10진수)
        App->>App: 로컬 안전 수칙 체크 (경고 알림 판단)
        
        par UI 표시 및 서버 전송
            App->>App: 대시보드 UI 업데이트
            App->>API: 서버 전송 (1~5초 분량 배치 업로드)
        end
    end

    Note over API, TSDB: [Phase 3] 적재 및 저장
    API->>TSDB: 텔레메트리 적재 (하이퍼테이블)
    
    opt 백엔드 로직에서 이상 감지 시
        API->>App: FCM 푸시 발송 (위험 경보)
    end
    deactivate OBD
```
