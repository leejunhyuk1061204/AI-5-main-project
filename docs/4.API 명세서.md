...
### 3.1 정비 이력 관리
- **POST `/vehicles/{vehicleId}/maintenance` (BE-MT-001)**: 정비 기록 등록
- **Request Body**:
  ```json
  {
  ...
  ```
- **Response**: `ApiResponse<MaintenanceHistoryResponse>`

- **GET `/vehicles/{vehicleId}/maintenance`**: 차량별 정비 이력 목록 조회
