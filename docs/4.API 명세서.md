...
### 3.1 정비 이력 관리
- **POST `/api/vehicles/{vehicleId}/maintenance` (BE-MT-001)**: 정비 기록 등록
- **Request Body**:
  ```json
  {
...
- **GET `/api/vehicles/{vehicleId}/maintenance`**: 차량별 정비 이력 목록 조회
- **Response**: `ApiResponse<List<MaintenanceHistoryResponse>>`

- **GET `/api/vehicles/{vehicleId}/consumables`**: 주요 소모품 상태 및 수명 예측 조회
...
- **GET `/notifications` (BE-NT-001) [구현 예정]**: 알림 센터 내역 조회
