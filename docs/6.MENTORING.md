...
| 테이블명 | 컬럼명 | 타입 | 제약조건 | 설명 |
|:---|:---|:---|:---|:---|
| **vehicle_consumables** | vehicle_consumable_id | BIGINT | PK | 식별자 |
| | vehicle_id | UUID | FK | 차량 식별자 |
| | consumable_id | BIGINT | FK | 소모품 항목 식별자 |
| | custom_interval_mileage | FLOAT | | 사용자 설정 교체 주기 (km) |
| | custom_interval_months | INT | | 사용자 설정 교체 주기 (개월) |
...
- [-] 이미지/소리/주행 데이터를 종합한 통합 진단 로직 구현
- [x] AI 분석 서버 연동
- [ ] 자동차 제조사 데이터 정기 동기화 구현
- [ ] 보안 토큰 관리 구현
- [ ] 위험도 판정 매트릭스(Critical~Low) 엔진 및 신뢰도 기반 스코어링 로직 구현
