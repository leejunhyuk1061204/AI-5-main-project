...
| 기능명 | Method | Endpoint | 설명 |
| :--- | :--- | :--- | :--- |
| 차량 등록 | POST | `/` | 사용자 입력 정보로 차량 등록 `[x]` |
| 차대번호 등록 (OBD) | POST | `/obd` | OBD VIN 정보를 이용한 단순 등록 `[x]` | - VIN으로 차량정보 가져오는 api 불확실. -> 차량등록 + 차대번호 등록 만 구현
| 차량 목록 조회 | GET | `/` | 사용자가 등록한 모든 차량 리스트 `[x]` |
| 차량 상세 조회 | GET | `/{vehicleId}` | 특정 차량의 상세 정보 조회 `[x]` |
| 차량 정보 수정 | PUT | `/{vehicleId}` | 차량 닉네임, 메모 등 수정 `[x]` |
| 대표 차량 설정 | PATCH | `/primary` | 메인 차량으로 지정 `[x]` |
| 차량 삭제 | DELETE | `/{vehicleId}` | 차량 정보 삭제 (Soft Delete) `[x]` |
